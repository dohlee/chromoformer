...
class Roadmap3D(Dataset):
    ...
    def _get_region_representation(
        ...
        x = torch.tensor(
            np.load(f"demo_data/{self.eid}/{chrom}:{start}-{end}.npy")
        ).float()
    ...
    def __getitem__(self, i):
        ...
        for bin_size, max_n_bins in [
            (2000, self.w_max // 2000),
            (500, self.w_max // 500),
            (100, self.w_max // 100),
        ]:
            x_pcres, mask_pcres = [], []
            ...
            for i, (score, pcre) in enumerate(zip(scores, pcres)):
                ...
                (
                    ...
                    chrom_pcre, start_pcre, end_pcre, bin_size, max_n_bins
                )
            ...
            item[f"pad_mask_pcre_{bin_size}"] = torch.stack(
                mask_pcres
            )  # i_max x max_n_bins x max_n_bins
